    ...
    test "redundant clause checking on structs with many fields" do
      files = %{
        ...
        defmodule BigStruct do
          ...
          # Clauses with value: nil and distinct single-key schema patterns.
          # The final clause has no value constraint, so it differs from all
          # preceding clauses in both :value and :schema fields. This prevents
          # the map_all_but_one optimization, causing a BDD chain to accumulate.
          # Without the fix, navigating to variable x requires pair_make_disjoint
          # on 20 mutually overlapping open maps, an O(2^20) operation.
